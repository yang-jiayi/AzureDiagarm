...
function serializeInlineScriptJson(value: object): string {
  const json = JSON.stringify(value);
  if (json === undefined) {
    throw new TypeError('Inline script data could not be serialized.');
  }
  return json
    .replace(/</g, '\\u003c')
    .replace(/\u2028/g, '\\u2028')
...
export function renderHtml(layout: LayoutResult, title?: string, options: RenderHtmlOptions = {}): string {
  ...
  const showCosts = profile === 'cost';
  const edgeSemantics = resolveRenderEdgeSemantics(layout, profile);

  // Serialize layout data for the client-side JS
  const layoutJson = serializeInlineScriptJson(layout);
  const edgeSemanticsJson = serializeInlineScriptJson({
    focusProfile: edgeSemantics.focusProfile,
    primary: [...edgeSemantics.primary],
    policyAssociations: [...edgeSemantics.policyAssociations],
    labeled: [...edgeSemantics.labeled],
  });
  ...
  if (showCosts && costedNodes.length > 0) {
    metaParts.push(`Fixed priced baseline: ~$${fmtCost(totalCost)}/mo (${esc(currency)}). Excludes ${rangeNodes.length} usage-based or ranged items shown on nodes.`);
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header .meta { font-size: 12px; line-height: 1.35; opacity: 0.88; margin-left: auto; max-width: min(44vw, 660px); text-align: right; overflow-wrap: anywhere; }
  ...
  body.profile-presentation .node .name { font-size: 14px; }
  body.profile-presentation .edge-label { font-size: 11px; }
  body.profile-presentation .group .group-label { font-size: 13px; }
...
<body class="${themeClass} profile-${profile}" data-render-profile="${profile}">
...
const showCosts = ${showCosts};
const edgeSemantics = ${edgeSemanticsJson};
const primaryEdgeKeys = new Set(edgeSemantics.primary);
const policyAssociationKeys = new Set(edgeSemantics.policyAssociations);
const labeledEdgeKeys = new Set(edgeSemantics.labeled);
...
function orthogonalRoute(edge, direction, obstacles, canvas) {
  ...
  const deltaX = Math.abs(t.x - s.x), deltaY = Math.abs(t.y - s.y);
  const routeDirection = deltaY > deltaX * 1.1 ? 'TB' : deltaX > deltaY * 1.1 ? 'LR' : direction;
  ...
  function midpointRoute() {
    var raw;
    if (routeDirection === 'LR') { var mx = (s.x + t.x) / 2; raw = [s, { x: mx, y: s.y }, { x: mx, y: t.y }, t]; }
...
function render() {
  ...
  layout.edges.forEach(e => {
    ...
    if (edgeSemantics.focusProfile) {
      path.classList.add(isPrimary ? 'edge-primary' : 'edge-supporting');
      path.style.opacity = isPrimary ? '1' : isPolicyAssociation ? '0.8' : '0.58';
      path.style.strokeWidth = isPrimary ? '2.4' : '1.2';
    }
  ...
  layout.nodes.forEach(n => {
    ...
    el.innerHTML =
      ...
      (showCosts && n.estimatedCost ? '<div class="cost">~$' + fmtCost(n.estimatedCost) + '/mo</div>'
        : showCosts && n.costRange ? '<div class="cost cost-range">' + esc(n.costRange) + '</div>' : '');
