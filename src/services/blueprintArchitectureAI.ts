...
C. Monitoring / observability / governance services (Azure Monitor, Log Analytics, Application Insights, Microsoft Sentinel, Defender for Cloud) also belong ABOVE the main pipeline OR in a dedicated Observability zone to the side. Their edges should be dashed (style: "dashed").
   C.1 Observability data-flow direction — telemetry, logs, metrics, and traces flow FROM the emitting service INTO the collection store. Collection stores do not emit telemetry to other collection stores. Specifically:
        • Application Insights → Log Analytics is the only direction used between these two (App Insights is backed by a Log Analytics workspace).
        • Workloads (Container Apps, Functions, AKS, App Service, VMs, API Management, Application Gateway, Front Door, etc.) → Log Analytics or Application Insights (not the reverse).
        • Workloads → Azure Monitor is fine for platform metrics with labels like "Publish metrics" or "Emit metrics".
        • Do not emit Azure Monitor → Log Analytics edges. Azure Monitor is a query and alerting surface that reads from Log Analytics; it does not push logs into it.
...
K. Identity / OAuth flow direction — Microsoft Entra ID is an identity provider that workloads delegate to; end users do not connect to Entra ID directly as their first hop unless Entra ID is the diagram's primary subject. For web apps, APIs, and any workload protected by Entra ID:
   • The user's first edge goes to the application entry point (Front Door, Application Gateway, API Management, App Service, Container Apps, etc.) — not directly to Microsoft Entra ID.
   • The application (or its ingress) is what calls Entra ID for sign-in or token validation. Use an edge FROM the workload TO Microsoft Entra ID with a label like "Authenticate user", "Validate token", or "Acquire token". This is the OAuth/OIDC redirect or on-behalf-of pattern, not a direct user-to-IdP step.
   • An edge 'user → Microsoft Entra ID' labeled "Sign in" is incorrect for any diagram whose subject is a web app, API, or enterprise workload. The correct shape is 'user → app-ingress' (step 1, label "Browse app" / "HTTPS request") then 'app-ingress → Microsoft Entra ID' (step 2, label "Authenticate user" / "Validate token").
