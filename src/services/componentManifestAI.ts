...
4. ALWAYS include an observability zone (Azure Monitor + Log Analytics, plus Application Insights for app workloads) and an identity zone (Microsoft Entra ID) for every production workload — not only when the prompt mentions them. Add Azure Key Vault whenever the workload handles secrets, keys, certificates, or connection strings.
