...
const CompareValidationModal: React.FC<CompareValidationModalProps> = ({
  ...
  const formatComparisonAsMarkdown = (): string => {
    const successful = results.filter(r => r.status === 'success');
    const date = new Date().toLocaleString(language === 'ja' ? 'ja-JP' : 'en-US');
    ...
    // Footer
    md += `*Report generated by Microsoft Product Architecture Diagram Builder*  \n`;
        ...
        <div className="compare-modal-body">
          ...
          <div className="cv-waf-info">
            <p className="cv-waf-intro">
              {' '}{localize(language, {
                en: "Each model validates your architecture against the Azure Well-Architected Framework — Microsoft's set of guiding tenets for improving the quality of cloud workloads across five pillars:",
                ja: '各モデルは、クラウド ワークロードの品質を高めるための Microsoft の5つの柱から成る指針「Azure Well-Architected Framework」に照らして、アーキテクチャを検証します。',
              })}{' '}</p>
